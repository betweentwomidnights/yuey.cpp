...
struct AutoregressiveControl {
    ...
    // though the model did not propose the end token. The history includes
    // the token just accepted, so a caller that needs to stop on a structural
    // boundary can look for one. reached_end is set, because a forced stop at
    // a boundary leaves a complete result rather than a truncated one.
